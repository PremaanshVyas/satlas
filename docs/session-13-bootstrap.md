...
- Stale-while-revalidate cache: key aussie-sky-catalog-v3, 24h TTL. Always serves from cache immediately; background refresh keeps it warm. Satellites never disappear.
...
KEY TECHNICAL STATE:
- celestrak.ts: cache key = 'aussie-sky-catalog-v3', MAX_CACHE_AGE_MS = 24h. parseTleText() + fetchFromCelesTrak() (primary, browser) + fetchFromRailway() (silent fallback). Stale-while-revalidate always.
