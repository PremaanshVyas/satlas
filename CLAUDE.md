...
**Current phase:** Session 16 in progress — AWS Foundation code complete. Terraform (3 waves), ECS orbital service rewrite, CI ECR push job, frontend CloudFront catalog, RDS schema all committed. Pending: user to run `terraform apply` after AWS CLI setup, then update Vercel env vars, set up Sentry, delete Railway service.
...
- [x] `satellites.py` rewritten: Space-Track 3LE fetch via `_fetch_space_track_tles()`, S3 write via `_s3_refresh()`, background `refresh_loop()` (startup retry every 30s then 2h steady-state)
- [x] `requirements.txt`: added boto3, sentry-sdk[fastapi], psycopg2-binary
- [x] `main.py`: Sentry init, startup event (run_migrations + refresh_loop), removed /satellites endpoint
- [x] `db.py`: psycopg2 migration runner; silent no-op if DATABASE_URL unset (local dev safe)
- [x] `migrations/001_initial.sql`: pgvector extension + subscribers table (UUID PK, email, lat/lon, norad_id, min_elev)
- [x] `infra/terraform/`: complete Terraform across 3 waves — ECR, IAM OIDC, Secrets Manager, VPC, ECS Fargate (256 CPU/512MB), ALB (HTTP port 80), S3 catalog bucket (public read), CloudFront (https-only, 2h TTL), RDS PostgreSQL 15.7 (db.t3.micro, private subnet, random password)
- [x] `.github/workflows/ci.yml`: `ecr-push` job — OIDC auth, assumes `aussie-sky-ci` role, pushes to ECR on main branch push only
- [x] `celestrak.ts`: `VITE_CATALOG_URL` env var for CloudFront catalog; falls back to /api/catalog
- [x] `.env.example`: documented VITE_CATALOG_URL
- [x] 79 orbital Python tests passing
...
| `docs/session-17-bootstrap.md` | Next session full context prompt — paste at start of Session 17. |
