...
**Current phase:** Post-Session-8 (+ arc GMST fix + chatbot position sync). ISS accuracy fixed (5-min TLE cache), NASA 8K textures, atmosphere + stars live, ISS dot now sits on the orbital ring. Chatbot `get_satellite_info` now uses same 5-min ISS TLE as globe — positions match.
