...
**Current phase:** Session 9 (in progress). Pre-session bugs fixed: CelesTrak FORMAT=TLE switch (catalog was silently broken), chatbot timezone (server-side Melbourne time now injected). Click-to-select live. Stale cache fallback added (satellites no longer disappear on source outage). Satellite heights now use actual propagated altitude (geo.height km → Earth-radii). 77 orbital + 32 frontend tests all green.
...
- **2026-05-13 — Architectural rule: Claude is the presenter, never the calculator.** Claude must not compute, infer, or guess any data value shown to the user — not time, not timezone offsets, not satellite positions, not pass windows, not orbital parameters. Every value Claude presents must come from a backend tool call result or a pre-computed value explicitly injected into the system prompt. Claude's only job is to take that data and format it clearly. If data is missing, Claude says it's unavailable — it must not fill the gap using its training knowledge. The timezone bug (chatbot saying May 14 when it was May 13 in Melbourne) was caused by violating this rule: we passed UTC time and let Claude infer the Melbourne offset instead of computing it server-side with `toLocaleString('en-AU', {timeZone: 'Australia/Melbourne'})`. Fix all future data presentation the same way: compute it, then hand Claude the answer to format.

- **2026-05-13 — Chatbot position data didn't match the globe: ISS TLE cache mismatch.** The globe's ISS dot uses the 5-min `/tle/iss` TLE cache (refreshed every 2 min by the frontend), while the chatbot's `get_satellite_info` tool called `get_satellites()` (30-min catalog cache) for ALL satellites including the ISS. When NASA issues a new ISS TLE, the globe picks it up within 5 min but the chatbot continued using the stale catalog TLE for up to 30 more min — potentially thousands of km of position error. Fix: `satellite_info(catalog, query, fresh_tles)` now accepts an optional `fresh_tles` dict (`{norad_id: {tle1, tle2}}`). The `/satellite-info` endpoint uses `asyncio.gather` to fetch `get_satellites()` and `get_iss_tle()` in parallel (both cached, so no extra latency), then passes `{ISS_NORAD_ID: iss_tle}` as `fresh_tles`. If the ISS TLE fetch fails, the endpoint falls back to catalog TLE gracefully. Also updated system prompt: "~1000-satellite catalog" → "~10,000-satellite catalog". Rule: any service that reports satellite position must use the same TLE freshness tier as the rendering layer for that satellite.

- **2026-05-14 — `_fetch_celestrak` was silently broken since Session 6 — GP JSON format never included TLE lines.** Root cause: `CELESTRAK_ACTIVE_URL` used `FORMAT=json`, which returns GP orbital elements (MEAN_MOTION, ECCENTRICITY, etc.) but NOT `TLE_LINE1`/`TLE_LINE2`. Our `_parse_gp()` function keyed on `TLE_LINE1` causing a `KeyError` on every real CelesTrak call. Tests passed because they mocked the response with fabricated JSON that included TLE keys — a textbook mock-divergence bug. In production, `_fetch_celestrak` always threw, `get_satellites()` always fell back to SpaceTrack, and as long as SpaceTrack was healthy the globe worked fine. When SpaceTrack started failing (session boundary deploy → cold start → network issue), there was no working primary, causing `/satellites` to return 503 and the globe to run ISS-only. Fix: switched `CELESTRAK_ACTIVE_URL` to `FORMAT=TLE` (3LE text format: name / TLE1 / TLE2 triplets), added `_parse_tle_text()` to parse it (same approach as `_fetch_iss_tle`), and kept `_parse_gp()` only for the SpaceTrack fallback which genuinely does include TLE_LINE1/TLE_LINE2. Also added `TestParseTleText` unit tests. Rule: integration tests must use the response format the real API returns — never synthesise fixture data with keys that differ from the real response schema.

- **2026-05-14 — Chatbot timezone bug fixed: Melbourne time now computed server-side.** The chatbot's system prompt passed UTC time and told Claude to infer the Melbourne offset itself. Claude guessed wrong (AEST vs AEDT confusion), showing May 14 when it was 10:35 PM May 13 Melbourne time. Fix: `buildSystemPrompt` now calls `now.toLocaleString('en-AU', { timeZone: 'Australia/Melbourne', dateStyle: 'full', timeStyle: 'long' })` server-side and injects the pre-computed Melbourne string alongside UTC. System prompt explicitly says "do not compute timezone offsets yourself." Rule: this is the architecture rule restated — Claude is the presenter, the server is the calculator. Never pass Claude a raw UTC timestamp and expect it to correctly localise.

- **2026-05-14 — Satellites disappeared repeatedly: brittle pipeline + stale cache fix.** Root cause of recurring disappearance: CelesTrak GROUP=active blocks Railway cloud IPs (confirmed in session 6), so `_fetch_celestrak` always throws. `get_satellites()` fell back to SpaceTrack, which works when healthy. On session-boundary deploys, SpaceTrack also had cold-start or transient failures, causing both sources to fail and `/satellites` to return 503. The frontend catches 503 gracefully (runs ISS-only), so no error is shown — just blank dots. Fix: added stale cache fallback in `get_satellites()` — if both sources fail AND `_cache['tles']` is populated from a prior successful fetch, the stale catalog is returned instead of raising. Satellites only disappear on fresh Railway deploys where no prior fetch succeeded. Rule: always serve stale data rather than 503 for catalog endpoints; stale satellite positions are preferable to no satellites.

- **2026-05-14 — Satellite heights corrected: geo.height instead of hardcoded radius.** Both `propagator.worker.ts` (all catalog dots) and `SatelliteMesh.ts` (ISS dot) used fixed radii (1.02 and 1.06 respectively) placing all satellites at the same visual height just above Earth's surface. satellite.js `eciToGeodetic` returns `geo.height` in km. Fix: `r = (6371 + geo.height) / 6371` in both files — satellites now render at their actual orbital altitude. LEO (ISS at 420km → r≈1.066), Starlink at 550km → r≈1.086, GPS at 20200km → r≈4.17, GEO at 35786km → r≈6.62. The ISS orbital arc (in `computeArcPoints`) already used `mag / R_EARTH_KM` from ECI propagation so was already correct. Camera maxDistance raised from 8 to 15, far plane from 100 to 200, so users can zoom out to see MEO and GEO shells. Rule: never hardcode radius values for satellite positions — always derive from propagated altitude.
...
ARCHITECTURE RULE (enforce this strictly going forward) — Claude's role in this platform is PRESENTER ONLY, never calculator. Claude must not compute, infer, or guess any data value — not time, not timezone offsets, not satellite positions, not pass windows, nothing. Every piece of data Claude presents to the user must have come from either: (a) a backend tool call result, or (b) an explicitly pre-computed value injected into the system prompt. Claude's only job is to take that data and format it clearly for the viewer. If data is missing, Claude says it's unavailable — it does not fill the gap with its own reasoning or training knowledge. This is why the timezone bug happened: we let Claude infer the Melbourne offset instead of computing it server-side and passing the answer in.
