...
- **2026-05-30 — Session 45: account reinstated; satcat brought into per-class compliance and its restart-storm hole closed.** Space-Track restored the account and pointed at their published per-class limits ([documentation](https://www.space-track.org/documentation), [usage notes](https://spacetrack.readthedocs.io/en/stable/usage.html)). Audit of our single client against those rules: global throttle (<30/min, <300/hr) — we're 3+ orders of magnitude under; `gp` (1/hour, off-peak minute) — already compliant (`:17`, hard `GP_MIN_INTERVAL_SECONDS` guard); comma-delimited multi-object lists — N/A, we bulk-pull the whole `gp` class by date which is the more efficient pattern; full-catalog stored locally / no re-download — already done (S3 seed on boot). **One real gap and one latent bug found in `satcat`:** their rule is once/day *after 1700 UTC*, but we ran a drifting 24h timer (right frequency, wrong time-of-day) AND `_satcat_last_refresh` was in-process only, so a crash-loop would re-query satcat on every boot — the exact restart-storm class that suspended us, just on the satcat endpoint. Fix: `_satcat_due(now, last)` allows a query only when it's a new UTC day and `hour >= SATCAT_QUERY_HOUR_UTC` (17), with a bootstrap exemption (`last<=0`) so a first-ever cold start still populates metadata; `refresh_loop` seeds `_satcat_last_refresh` from `satcat.json`'s S3 `LastModified` (`_satcat_last_modified_from_s3`, a `head_object`) so restarts inherit the day's clock. Also corrected stale "every 2 hours" wording in `ApiDocs.tsx`. **Follow-up the same session (caught while verifying the `:17` cadence during recovery):** the `gp` clock `_last_gp_query_at` had the *same* in-process-reset weakness — it was never seeded from S3, so it relied solely on the fixed `:17` minute. That's safe in steady state, but an off-schedule **bootstrap** query (which writes at boot time, not `:17`) followed by a redeploy that boots before the next `:17` would issue a second `gp` query within the hour (we hit exactly this during recovery and only avoided it because the redeployed worker happened to boot *after* `:17` — verified via `catalog.tle`'s unchanged S3 LastModified). Fix: extracted `_seed_from_s3()` and seeded `_last_gp_query_at` from `catalog.tle`'s S3 LastModified (= last successful gp write) so `_next_gp_slot` enforces the once/hour gap across restarts too. Tests: +10 total this session (`TestSatcatDue`, `TestSatcatLastModifiedFromS3`, `TestSeedFromS3` incl. a cross-restart double-query regression) → 113 orbital; 169 web; typecheck clean. For future query-shape experiments use Space-Track's test server `for-testing-only.space-track.org`, not production. Rule: a once-per-day limit means once *per calendar day at the vendor's stated hour*, not "≥24h since last call"; and EVERY rate-limited clock (gp and satcat alike) must be seeded from durable storage on boot, never left as in-process state, or a restart/redeploy silently reproduces the abuse — fixing one endpoint and not its sibling is the trap.
