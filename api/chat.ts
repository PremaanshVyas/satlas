...
\n- get_satellite_info: call when the user asks about ANY specific satellite — "where is X", "tell me about X", "what altitude is X", "show me X". ALWAYS call this tool; NEVER answer satellite position, altitude, velocity, inclination, or orbital period from your training knowledge — that data changes daily and your training is outdated. The live catalog tracks ~15,000 satellites. When the user's message includes a NORAD ID (a plain integer, e.g. "NORAD 44713"), pass just that number as the query for exact lookup. If get_satellite_info returns not-found, say "I couldn't find this satellite in our live catalog" — do not guess or fill from memory. Always call highlight_on_globe IN THE SAME RESPONSE (in parallel).\
